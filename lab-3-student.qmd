...
- These data have 30,463 rows and 15 columns.

**2. What are the data types of the variables in this dataset?**

- There are 7 numeric, 7 character, and 1 date data types for the variables.
...
ggplot(data = surveys) +
  geom_point(aes(x = weight, y = hindfoot_length, color = species), alpha = 0.25) +
  facet_wrap(~species, scales = "free") +
  labs(x = "Weight of Animal (g)", y = "", subtitle = "Length of Hindfoot (mm)", title = "Relationship between weight and hindfoot length in species \nobserved by the Portal Project LTER") +
...
ggplot(aes(x = species, y = weight), data = surveys) +
  geom_jitter(color = "darkseagreen", alpha = 0.2) +
  geom_boxplot(alpha = 0) +
  labs(x = "Species of Animal", y = "Weight of Animal (g)", title = "Distributions of the weights of each species") +
  theme(axis.text.x = element_text(angle = 315, hjust = -0.15)) #https://stackoverflow.com/questions/1330989/rotating-and-spacing-axis-labels-in-ggplot2
...
ggplot(aes(x = weight, y = species), data = surveys) +
  geom_jitter(aes(color = species), alpha = 0.2) +
  geom_boxplot(alpha = 0) +
  labs(y = "Species of Animal", 
       x = "Weight of Animal (g)", 
       title = "Distributions of the weights of each species") +
  scale_color_manual(values = c("darkseagreen", "slateblue2", "aquamarine3", "cadetblue3", "forestgreen", "turquoise", "royalblue", "seagreen2", "skyblue2", "lightseagreen", "powderblue", "darkgreen", "navy", "lightblue")) +
  guides(color = "none")
